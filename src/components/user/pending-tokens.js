...
import { Box, Button, Container, List, Typography, Tooltip, IconButton } from '@mui/material';
...
const PendingTokens = (props) => {
  ...
  async function initData(tokenHash) {
    ...
    const tknD = await tracking.methods
      .getTokenData(tokenHash)
      .call()
      .then(async (token) => {
        ...
        if (Number(token[1]) == 2) {
          ...
          data = await axios.get(`${process.env.NEXT_PUBLIC_BACKEND_URL}/api/v1/ingridient/` + token[0]);

          setDataRows((previousRow) => [
            ...previousRow,
            {
              id: idCounter,
              tokenhash: tokenHash,
              tokenid: data.data.data[0]?.name,
              tokensender: userName,
              tokenquantity: milligramsToKilograms(token[2]),
            },
          ]);
